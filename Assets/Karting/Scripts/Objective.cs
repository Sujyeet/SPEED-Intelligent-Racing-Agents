...
public abstract class Objective : MonoBehaviour
{
    ...
    protected void Register()
    {
        ...
        // register this objective in the ObjectiveHUDManger if present
        m_ObjectiveHUDManger = FindObjectOfType<ObjectiveHUDManger>();
        if (m_ObjectiveHUDManger != null)
            m_ObjectiveHUDManger.RegisterObjective(this);

        // register this objective in the NotificationHUDManager if present
        m_NotificationHUDManager = FindObjectOfType<NotificationHUDManager>();
        if (m_NotificationHUDManager != null)
    ...
    public void CompleteObjective(string descriptionText, string counterText, string notificationText)
    {
        ...
        // unregister this objective safely from both HUD managers if present
        if (m_ObjectiveHUDManger != null)
    ...
    public void UnregisterPickup(TargetObject pickupCollected)
    {
        ...
        {
            Debug.Log($"[Objective] Mismatch gameMode! Pickup is {pickupCollected.gameMode}, Objective is {gameMode}");
            return;
        }
        ...
        if (pickupCollected.gameMode == GameMode.Laps)
        {
            ...
            {
                Debug.Log($"[Objective] First pass on finish line - arming lap counter.");
                TimeDisplay.OnUpdateLap?.Invoke();
                lapObject.lapOverNextPass = true;
                return;
            }

            int activeRemaining = NumberOfActivePickupsRemaining();
            Debug.Log($"[Objective] Finish line pass! Active remaining pickups: {activeRemaining}");
        ...
        else
        {
            ...
            if (gameMode == GameMode.Laps)
                KartGame.Track.TimeDisplay.OnUpdateLap?.Invoke();
        }
    }
