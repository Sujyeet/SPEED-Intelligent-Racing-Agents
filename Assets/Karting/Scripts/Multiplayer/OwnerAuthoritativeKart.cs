...
public class OwnerAuthoritativeKart : NetworkBehaviour
{
    ...
    private void Update()
    {
        if (!IsOwner)
        {
            // Non-owner: Smooth to received position IN UPDATE for buttery smooth rendering!
            // Only move if Rigidbody is kinematic (otherwise physics handles it)
            if (rb.isKinematic)
            {
                tf.position = Vector3.Lerp(tf.position, targetPosition, Time.deltaTime * 15f);
                tf.rotation = Quaternion.Slerp(tf.rotation, targetRotation, Time.deltaTime * 15f);
            }
        }
    }
