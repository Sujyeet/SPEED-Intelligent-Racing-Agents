    ...
    void Update()
    {
        if (gameState != GameState.Play)
        {
            ...
            if(elapsedTimeBeforeEndScene >= endSceneLoadDelay)
            {
                ...
                {
                    if (!string.IsNullOrEmpty(m_SceneToLoad) && Application.CanStreamedLevelBeLoaded(m_SceneToLoad))
                    {
        ...
        {
            if (m_ObjectiveManager != null && m_ObjectiveManager.AreAllObjectivesCompleted())
                EndGame(true);

            if (m_TimeManager != null && m_TimeManager.IsFinite && m_TimeManager.IsOver)
                EndGame(false);
        }
    }
