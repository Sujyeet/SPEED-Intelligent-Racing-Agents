...
public class ObjectiveCompleteLaps : Objective
{
    ...
    protected override void ReachCheckpoint(int remaining)
    {
        ...
        currentLap++;
        Debug.Log($"[ObjectiveCompleteLaps] Lap completed! Current lap: {currentLap} / {lapsToComplete}");

        // Notify MultiplayerRaceManager if present
        if (KartGame.Multiplayer.MultiplayerRaceManager.Instance != null)
        {
            KartGame.Multiplayer.MultiplayerRaceManager.Instance.OnLocalPlayerProgress(currentLap, 0);
        }
        ...
        {
            Debug.Log("[ObjectiveCompleteLaps] Laps completed! Triggering WinScene...");
            
            GameFlowManager gameFlow = FindObjectOfType<GameFlowManager>();
            if (gameFlow != null)
            {
                gameFlow.SendMessage("EndGame", true, SendMessageOptions.DontRequireReceiver);
            }
